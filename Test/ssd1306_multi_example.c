...
// 第5~6字节表示图像高度
// 此处被注释掉了
const static unsigned char gImage_1[512] = { /* 0X32,0X01,0X00,0X40,0X00,0X40, */
...
{
	// ssd1306_Fill_all();
  ssd1306_multi_Write_String(OLED_0, 0, 0, (uint8_t *)"DRAGON 0");//在ssd1306屏幕的X轴为0，Y轴为0，显示字符串“DRAGON”
	ssd1306_multi_Write_String(OLED_1, 0, 0, (uint8_t *)"DRAGON 1");//在ssd1306屏幕的X轴为0，Y轴为0，显示字符串“DRAGON”
	delay_s(2);
  ssd1306_multi_Write_String(OLED_0, 21, 3, (uint8_t *)"Hello 2022");
	ssd1306_multi_Write_String(OLED_1, 21, 3, (uint8_t *)"Hello 2023");//在ssd1306屏幕的X轴为21，Y轴为3，显示字符串“Hello 2023”，不够显示自动换行
  delay_s(2);
...
{
	ssd1306_multi_DrawArrImage(OLED_0, 32, 0, 95, 63, (uint8_t *)gImage_1);
	ssd1306_multi_DrawArrImage(OLED_1, 32, 0, 95, 63, (uint8_t *)gImage_1);
	delay_ms(1000);
	ssd1306_multi_Clear(OLED_0);
	ssd1306_multi_Clear(OLED_1);
	delay_ms(1000);
